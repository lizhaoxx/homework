...
/*============================ PROTOTYPES ====================================*/
static fsm_rt_t check_engine(const msg_t *ptMSG,bool* pbIsCheckFail);
...
fsm_rt_t msg_map_search(const msg_t **ptSuccessMsg)
{
    fsm_rt_t tRes;
    uint8_t chByte = 0;
    ...
    switch(s_tState) {
        ...
        case TASK_CHECK_USE_PEEK_CHK_STR:
            tRes = check_engine(c_ptMsg,&bIsCheckFail);
            if(tRes == fsm_rt_cpl) {
