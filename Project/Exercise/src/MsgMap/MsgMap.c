...
static byte_queue_t *s_tMessageQuePTR = NULL;
...
fsm_rt_t MsgMapSearch(byte_queue_t *ptMessageQueue,const msg_t **ptSuccessMsg)
{
    fsm_rt_t tRes;
    uint8_t chByte = 0;
    ...
    switch(s_tState) {
        case TASK_CHECK_USE_PEEK_START:
            if(ptMessageQueue && ptSuccessMsg) {
                s_tMessageQuePTR = ptMessageQueue;
            } else {
                return fsm_rt_err;
            }
        ...
        case TASK_CHECK_USE_PEEK_SEARCH:
            if ( s_chMsgIndex < UBOUND(c_tMSGMap) ) {
                ...
                DEQUEUE_BYTE(s_tMessageQuePTR,&chByte);
                s_chMsgIndex = 0;
            }
            break;
...
static fsm_rt_t CheckEngine(const msg_t *ptMSG,bool* pbIsCheckFail)
{
    ...
    switch(s_tState) {
        case TASK_CHECK_ENGINE_START:
            if(INIT_CHK_STRING(&s_tCHK,ptMSG->chMSG,s_tMessageQuePTR)) {  
 ...
 */
bool add_dynamic_command(msg_t * ptMsg,uint8_t chItemNum)
{
